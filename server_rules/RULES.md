...
- API 재시도 로직 추가 전, 서버 캐시 레이어 동작을 반드시 확인한다. `fast=true` 등 캐시 우선 파라미터는 UNAVAILABLE 포함 캐시 히트를 반환하므로, 재시도는 캐시 우회(`fast=false` 등) 파라미터를 사용해야 효과가 있다. 캐시 우회 없는 재시도는 무한 UNAVAILABLE을 만든다.
